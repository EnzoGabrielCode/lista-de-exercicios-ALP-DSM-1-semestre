...
# near_ten(19) -> True
def near_ten(n):
  if (n - 2) % 10 == 0 or (n + 2) % 10 == 0 or (n - 1) % 10 == 0 or (n + 1) % 10 == 0 or n % 10 == 0:
    return True
  else:
    return False
...
def end_other(a, b):
  a = a.lower()
  b = b.lower()
  if a.count(b) >= 1 and a[-1] == b[-1]:
    return True
  elif b.count(a) >= 1  and a[-1] == b[-1]:
    return True
  else:
    return False
